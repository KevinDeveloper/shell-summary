...
loadImageMirro(){
    ...
    docker load -i $parentBasePath/images/bbox_nginx_prod.tar 
...
#   cp -r $parentBasePath/package/bbox-wechat-web.zip $installPath/bbox-wechat-web
	...
	cp -r $parentBasePath/package/web-wechat-sso-*.zip $installPath/saas-user-center
...
#	unzip $installPath/saas-user-center/saas-user-center.zip -d $installPath/saas-user-center>>/dev/null
...
#	unzip $installPath/bbox-wechat-web/bbox-wechat-web.zip -d $installPath/bbox-wechat-web >>/dev/null        
	...
	unzip $installPath/saas-user-center/web-wechat-sso-*.zip -d $installPath/saas-user-center>>/dev/null
...
createContainer(){
   ...
   docker create --name bbox_nginx --net host -v /etc/localtime:/etc/localtime:ro -v $LOG_PATH:/var/log/nginx -v /etc/nginx/conf.d/:/etc/nginx/conf.d/ -v $installPath/opnext-web:$installPath/opnext-web -v $installPath/bbox-visitor-web:$installPath/bbox-visitor-web -v $installPath/bbox-VSM:$installPath/bbox-VSM -v $installPath/saas-user-center:$installPath/saas-user-center -v $installPath/oms-web:$installPath/oms-web -v $installPath/bbox-wechat-web:$installPath/bbox-wechat-web  bbox_nginx:prod
   sleep 2
