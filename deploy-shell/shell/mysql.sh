...
MYSQL_ROOT_PASSWORD=$(cat $parentBasePath/docker.conf | grep "BB_MYSQL_PASSWORD" | awk -F '=' '{print $2}')
...
createContainer(){
   ...
   docker create --name bbox_mysql --net host --env-file $parentBasePath/docker.conf -v /etc/localtime:/etc/localtime:ro -v /etc/conf:/etc/mysql/conf.d -v $LOG_PATH/mysql:/var/log/mysql -v ${DATA_PATH}/mysql:/var/lib/mysql -e MYSQL_ROOT_PASSWORD='123456' bbox_mysql:prod
   sleep 10
...
executeSql(){
	...
	docker cp  $parentBasePath/sql/door-keeper.sql  bbox_mysql:/home
	docker cp  $parentBasePath/sql/o-service.sql  bbox_mysql:/home
	docker cp  $parentBasePath/sql/store.sql  bbox_mysql:/home
	docker cp  $parentBasePath/sql/user-center.sql  bbox_mysql:/home
    docker cp  $parentBasePath/sql/bpush.sql  bbox_mysql:/home
    docker cp  $parentBasePath/sql/visitor-job.sql  bbox_mysql:/home
