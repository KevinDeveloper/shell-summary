...
createContainer(){
   ...
   docker create --name bbox_redis --net host -v /etc/localtime:/etc/localtime:ro -v ${DATA_PATH}:/data bbox_redis:prod redis-server --appendonly yes   --requirepass "$REDIS_PASSWORD"
   sleep 2
