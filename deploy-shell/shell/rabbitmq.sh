...
BB_RABBITMQ_USERNAME=$(cat $parentBasePath/docker.conf | grep "BB_RABBITMQ_USERNAME" | awk -F '=' '{print $2}')
BB_RABBITMQ_PASSWORD=$(cat $parentBasePath/docker.conf | grep "BB_RABBITMQ_PASSWORD" | awk -F '=' '{print $2}')
...
createContainer(){
   ...
   docker create --name bbox_rabbitmq --net host --env-file $parentBasePath/docker.conf -v /etc/localtime:/etc/localtime:ro -e RABBITMQ_DEFAULT_USER=$BB_RABBITMQ_USERNAME -e RABBITMQ_DEFAULT_PASS=$BB_RABBITMQ_PASSWORD bbox_rabbitmq:prod
   sleep 2
