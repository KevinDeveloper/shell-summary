...
createContainer(){
   ...
   docker create --name consul --env-file $parentBasePath/docker.conf --net host -v /etc/localtime:/etc/localtime:ro -v $installPath/consul:/usr/local/consul java8:v1 sh /usr/local/consul/start.sh
   sleep 2
